...
\end{problem}
\begin{solution} \ \\
Seeking a contradiction, suppose that the $i^{th}$ box has less than $n_i$ objects. Let the $i$th box contain $n_i - 1$ objects. The total number of objects will then be $(n_1-1) + (n_2-1) + \dots + (n_1 - 1)$ objects. Summed together, this results in $n_1 + n_2 + \dots -t$. This contradicts $n_1 + n_2 + · · · + n_t - t + 1$
\end{solution}
